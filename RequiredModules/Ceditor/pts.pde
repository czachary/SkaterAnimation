...
class pts // class for manipulaitng and displaying pointclouds or polyloops in 3D 
 ...
 void savePts(String fn) 
  ...
  void displayCurve() 
    ...
    //MODULE 3: Display Skater
    void displaySkater(pt[] B)
    {
      pt basePoint = P(B[currstep], t, B[n(currstep)]);
      pt origCurvePoint = P(G[currstep], t, G[n(currstep)]);
      
      int currLeftStep = n(currstep);
      int currRightStep = p(currstep);
      
      pt leftFoot = P(B[currLeftStep], t, B[n(currLeftStep)]);
      pt rightFoot = P(B[currRightStep], t, B[n(currRightStep)]);
